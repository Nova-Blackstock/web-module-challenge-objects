...
console.log(createMenuItem('Steak Fajita', 15, "Dinner"));
console.log(createMenuItem('Chili Dog', 8, 'Lunch'));
console.log(createMenuItem('Margarita', 5, 'Drinks'));
...
function addFeedback(name, rating, feedback){
  reviews.push({name, rating, feedback});
  return reviews
};

console.log(addFeedback('Karen', 1, 'They made my dish slightly wrong, there was a homeless man who was coughing their lungs out right by the door and probably had COVID, and the manager was apparently there but none of the employees knew where he was. I asked several times.'));
...
function updateFeedback(index, comment){
  reviews[index].feedback = comment;
  return reviews[index]
};

console.log(updateFeedback(7, 'this place is chill with really cool people, great for getting work done on weekdays'));
...
For example, if getLastReview is invoked passing the reviews array it will return
 `Reyna gave the restaurant a 3.5 star review and their feedback was: "this place is chill with really cool people, great for getting work done on weekdays"`.
*/
...
*/
 ...
 function getReviewByRating(arr, num) {
    ...
    for(i = 0; i< arr.length; i++){
      if(Math.round(arr[i].rating) === num){
        newarr.push(arr[i]);  
      }; 
    }; return newarr
  };
...
*/
  function getLongReviews(arr) {
    const arr2 = [];
    for(i = 0; i< arr.length; i++){
      if(arr[i].feedback.split(/\s+/).length > 15){
        arr2.push(arr[i]);
      };
    }; return arr2
  };
...
function carMaker(miles) {
    ...
    return car.drive(500)
  };
    
console.log(carMaker(200))
